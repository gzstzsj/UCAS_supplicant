...
/*
QString cleantext(GumboNode* node) 
{
    if (node->type == GUMBO_NODE_TEXT) {
        ...
        node->v.element.tag != GUMBO_TAG_STYLE) {
            ...
            for (unsigned int i = 0; i < children->length; ++i) {
                const QString text = cleantext((GumboNode*) children->data[i]);
                    if (i != 0 && !text.isEmpty()) {
                    contents.append(" ");
                }
...
/*
QString read_info_gumbo_1(char* input)
{
    //QString ret_str = (gbk2utf(cleantext(output->root)));
    char out[MAXLINE];
    char * conved = out;
    size_t tmp1 = MAXLINE, tmp2 = MAXLINE;
    iconv_t convertor;
    convertor = iconv_open("utf8", "gb18030");
    //QTextCodec *gbk = QTextCodec::codecForName("GB18030");
    //QString ret_str = gbk->toUnicode(input);
    iconv(convertor, &input, &tmp1, &conved, &tmp2);
    printf("%d\t%d\n", tmp1, tmp2);
    iconv_close(convertor);
    printf("%s\n", out);
    GumboOutput* output = gumbo_parse(out);
    QString ret_str = cleantext(output->root);
...
/*
int read_info_bare_1(char* input)
{
    char * conved = info_text;
    size_t tmp1 = MAXLINE, tmp2 = MAXLINE;
    iconv_t convertor;
    convertor = iconv_open("utf8", "gb18030");
    iconv(convertor, &input, &tmp1, &conved, &tmp2);
